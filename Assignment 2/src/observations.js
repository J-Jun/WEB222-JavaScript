...
function observationSummary(data) {
  ...
  for (var i = 0; i < data.per_page; i++) {
    ...
    console.log(`#${data.results[i].id} - ${data.results[i].species_guess} (${data.results[i].created_at_details.date})`)
  }
}
...
function observationSummary2(data) {
  ...
  data.results.forEach(function(result){
    ...
    console.log(`#${result.id} - ${result.species_guess} (${result.created_at_details.date})`)
  }); 
}
...
function observationsByGeoPrivacy(data, geoPrivacy) {
  ...
  if (typeof geoPrivacy == 'string' && geoPrivacy != null) {
    ...
    for (var eachData of data.results) {
      geoPrivacy = geoPrivacy.toLowerCase();
      if (eachData.geoprivacy === geoPrivacy)
        observationArray.push(eachData);
    }
  }
  else {
    if (geoPrivacy == null) {
...
function transformObservation(original) {
  ...
  var photoArray = original.photos.map(photoElement => photoElement.url);

  // Modifying coordinates 
  var coordArray = original.location.split(",");
  for (var i = 0; i < coordArray.length; i++){
    coordArray[i] = parseFloat(coordArray[i]);
  }
  var coordTemp = coordArray[0];
  coordArray[0] = coordArray[1];
  coordArray[1] = coordTemp;
...
function transformObservations(data) {
  ...
  data.results.forEach(perData => transformedArray.push(transformObservation(perData)))
...
function transformObservations2(data) {
  ...
  var transformedArray = data.results.map(perData => transformObservation(perData));
...
function getObservationsById(data, ...ids) {
  ...
  if (ids.length > 1) {
    observationResult = new Array();

    ids.forEach(function(perID) {
      var idFound = data.results.find(idElement => idElement.id == perID);
      if (idFound) {
        observationResult.push(idFound);
      }
    });
  }
  else {
    ...
    var idFound = data.results.find(idElement => idElement.id == ids);
    if (idFound) {
      observationResult = idFound;
    }
  }
...
function getObservationsByPositionalAccuracy(data, options = {}) {
  ...
  if (Object.keys(options).length != 0) {
    ...
    if (options.min && options.max && options.equal) {
      arrObservation = data.results.filter(arrData => arrData.positional_accuracy === options.equal);
    }
    else if (options.min && options.max) {
      arrObservation = data.results.filter(arrData => (arrData.positional_accuracy >= options.min && arrData.positional_accuracy <= options.max));
    }
    else if (options.equal) {
      arrObservation = data.results.filter(arrData => arrData.positional_accuracy === options.equal);
    }
    else if (options.min) {
      arrObservation = data.results.filter(arrData => arrData.positional_accuracy >= options.min);
    }
    else if (options.max) {
      arrObservation = data.results.filter(arrData => arrData.positional_accuracy <= options.max);
    }
    else{
      arrObservation = [];
    }
...
function getTaxonPhotos(data) {
  ...
  data.results.forEach(function(photoElement){
    if (photoElement.taxon && photoElement.taxon.default_photo) {
      photoObject.original = `https://static.inaturalist.org/photos/${photoElement.id}/original.jpg`

      photoObject.square = `https://static.inaturalist.org/photos/${photoElement.id}/square.jpg`

      photoObject.small = `https://static.inaturalist.org/photos/${photoElement.id}/small.jpg`

      photoObject.medium = `https://static.inaturalist.org/photos/${photoElement.id}/medium.jpg`

      photoObject.large = `https://static.inaturalist.org/photos/${photoElement.id}/large.jpg`
...
function getUserStats(data) {
  ...
  for (i in data.results) {
    ...
    finalObject.totals.observations += data.results[i].user.observations_count;
    finalObject.totals.identifications += data.results[i].user.identifications_count;
    finalObject.totals.species += data.results[i].user.species_count;
    finalObject.count++;
  }

  finalObject.averages.observations = (finalObject.totals.observations / finalObject.count);
  finalObject.averages.identifications = (finalObject.totals.identifications / finalObject.count);
  finalObject.averages.species = (finalObject.totals.species / finalObject.count);
