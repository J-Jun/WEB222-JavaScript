 ...
 * 
 * // A string is passed as the parameter
 * // Removes leading adn trailing whitespace
 * // Convert to uppercase
 * // Space, tabs, or dots --> All of them are replaced with a single dash (-)
 * // regexp
 *
...
function kebab(value) {
  ...
  value = value.trim();
  // Switching every character to upper case 
  value = value.toUpperCase();
  // Replacing each space, tab, and dots to dashes
  value = value.replace(/\s|\t|\./g, '-');
  // Joining the multiple dashes into one
...
function createImg(src, alt, width) {
  ...
  if (width && !isNaN(width)) {
    finalString = `<img src="${src.trim()}" alt="${alt.trim()}" width="${width}">`;
  }
  else {
    finalString = `<img src="${src.trim()}" alt="${alt.trim()}">`;
  }
 ...
 * (lat, lng) 
 * 
 * If includebrackets is true, then you add [] around the lat and lng
 *
 * For example:
 *
...
function formatCoord(lat, lng, includeBrackets) {
  ...
  if (validateCoord(lat, lng)) {
    ...
    finalString = 'Invalid value for the coordinate';
  }
...
function formatCoords(...values) {
  ...
  for (var i = 0; i < values.length; i += 2) {
    indexValue.push(formatCoord(values[i], values[i+1]));
  }

  if (indexValue.length != 0){
    for (var i = 0; i < indexValue.length; i++) {
      indexString += `[${indexValue[i]}]`;
    ...
    indexString = 'Invalid arguments given';
  }
...
function generateLicenseLink(licenseCode) {
  ...
  var startString = '<a href="https://creativecommons.org/licenses/by-nc/4.0/">';
  var licenseString = '';
  ...
  switch(licenseCode) {
    ...
    case 'by':
      licenseString = 'Creative Commons Attribution License';
      break;
    case 'by-nc':
      licenseString = 'Creative Commons Attribution-NonCommercial License';
      break;
    case 'by-sa':
      licenseString = 'Creative Commons Attribution-ShareAlike License';
      break;
    case 'by-nd':
      licenseString = 'Creative Commons Attribution-NoDerivs License';
      break;
    case 'by-nc-sa':
      licenseString = 'Creative Commons Attribution-NonCommercial-ShareAlike License';
      break;
    case 'by-nc-nd':
      licenseString = 'Creative Commons Attribution-NonCommercial-NoDerivs License';
      break;
    default:
      startString = '<a href="https://choosealicense.com/no-permission/">';
      licenseString = 'All Rights Reserved';
      break;
  }

  return (startString + `${licenseString}</a>`);
}
  ...
  if (typeof(value) == 'string'){
    ...
    switch(value) {
      ...
      case 'undefined':
        ...
        break;
      default:
        boolField = 'invalid boolean value';
        break;
    }
  }
...
function all() {
  ...
  // Moving the arguments to a new array
  for (var i = 0; i < arguments.length; i++) {
    newArray.push(arguments[i]);
  }

  finalBool = newArray.every(element => pureBool(element));  
...
function buildUrl(q, perPage, order, license, lat, lng) {
  ...
  // Query 
  var qString = `?q=${encodeURIComponent(q)}`;

  // Number of results per page (between 1-200)
  if (perPage < 1 || perPage > 200){
    throw 'Wrong number of pages';
  }
  else {
    var pageString = `&per_page=${perPage}`;
  }
  ...
  if (order != 'asc' || order != 'desc') {
    ...
    var sortString = `&order=${order}`;
  }

  // License
  var licenseArray = ['none', 'any', 'cc-by', 'cc-by-nc', 'cc-by-sa', 'cc-by-nd', 'cc-by-nc-sa', 'cc-by-nc-nd']

  if (!licenseArray.includes(license)) {
    throw 'Invalid license';
  }
  else 
    var licenseString = `&license=${license}`;
  ...
  if (!validateCoord(lat, lng)) {
    ...
    var coordString = `&lat=${lat}&lng=${lng}`

  return `https://api.inaturalist.org/v1/observations${qString}${perPage}${sortString}${licenseString}${coordString}`;
}
